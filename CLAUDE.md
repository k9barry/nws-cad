...
PHP 8.3 system that monitors a `var/watch/` folder for NWS Aegis CAD XML files, parses them into a normalized 15-table schema (MySQL or PostgreSQL via `DB_TYPE`), exposes the data through a REST API and a real-time web dashboard, and emits ntfy.sh / Pushover notifications when calls are created/updated.
